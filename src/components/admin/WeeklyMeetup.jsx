...
import { createmeetup, deletmeetup, editmeetup, getallmeetup } from '../../store/Actions/adminActions';
...
const WeeklyMeetup = () => {
    ...
    const AddEventHandler = (e) => {
        ...
        setLoading(true);
        if (isEditing) {
            formDataToSend.append("meetupid", editId);
            dispatch(editmeetup(formDataToSend)).then(() => {
                dispatch(getallmeetup()).then((data) => {
                    setallforms(data || []);
                    setLoading(false);
                });
            });
        } else {
            dispatch(createmeetup(formDataToSend)).then(() => {
                dispatch(getallmeetup()).then((data) => {
                    setallforms(data || []);
                    setLoading(false);
                });
                                ...
                                <tr key={index}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                        ...
                                        <RiDeleteBin2Fill
                                            onClick={() => {
                                                setLoading(true);
                                                dispatch(deletmeetup(form._id)).then(() => {
                                                    dispatch(getallmeetup()).then((data) => {
                                                        setallforms(data || []);
                                                        setLoading(false);
                                                    });
